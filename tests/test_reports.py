...
from src.reports import json_write, report_writer, spending_by_category


@patch("src.reports.json_write")
...
def test_write_json_with_dataframe(mock_open, mock_makedirs):
    ...
    json_write("test_report.json", test_df)
...
def test_write_json_with_dict(mock_open, mock_makedirs):
    ...
    json_write("test_report.json", test_data)
    mock_makedirs.assert_called_once_with(".", exist_ok=True)

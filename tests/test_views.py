...
def test_main_page_returns_json_string(
    ...
    """Тестируем, что функция возвращает JSON строку"""
    # Мокаем datetime
    mock_now = Mock()
    mock_now.hour = 10
    ...
    mock_date_pars = Mock()
    mock_date_pars.year = 2024
    mock_date_pars.month = 12
    ...
    mock_datetime.strftime.return_value = "2024-12-07 14:00:00"

    # Мокаем функции
    mock_get_date.side_effect = lambda x: {"2025-09-30 20:00:15": "30.09.2025", "2024-12-07 14:00:00": "07.12.2024"}[x]
    ...
    mock_load_json_data.return_value = {"user_currencies": ["USD", "EUR"], "user_stocks": ["MSFT", "TSLA"]}

    mock_get_greeting.return_value = "Добрый день!"
    mock_get_card_infos.return_value = [{"last_digits": "1111", "total_spent": 2000.0}]
    mock_get_top_transactions.return_value = [{"date": "07.12.2024", "amount": -200.0}]
    mock_get_exchange_rate.return_value = [{"currency": "USD", "rate": 95.5}]
    mock_get_stock.return_value = [{"stock": "MSFT", "price": 350.00}]

    # Вызываем функцию
    result = main_page("2025-09-30 20:00:15")
...
def test_json_structure_and_content(
    ...
    mock_get_date.side_effect = lambda x: "07.12.2024" if "2024-12-07" in x else "30.09.2025"
    ...
    mock_load_json_data.return_value = {"user_currencies": ["USD"], "user_stocks": ["MSFT"]}

    mock_get_greeting.return_value = "Доброе утро!"
    mock_get_card_infos.return_value = [{"last_digits": "5555", "total_spent": 1000.0}]
    mock_get_top_transactions.return_value = [{"date": "15.01.2023", "amount": -200.0}]
    mock_get_exchange_rate.return_value = [{"currency": "USD", "rate": 95.5}]
    mock_get_stock.return_value = [{"stock": "TSLA", "price": 450.0}]
